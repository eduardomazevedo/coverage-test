...
chunk_size_table <-
  ...
  mutate(
    ...
    n_chunks = 1000 / chunk_size
  ) |>
  select(model_type, softmax_correction, n_obs, chunk_size, n_chunks)
...
create_job_chunks <- function(model_type, n_obs, n_bootstraps, softmax_correction, heritability_source) {
  ...
  total_chunks <- match$n_chunks[1]
  
  chunks <- map(1:total_chunks, function(i) {
    list(
      model_type = model_type,
      n_obs = n_obs,
      n_bootstraps = chunk_size,
